function DateSlider(paramObj)
{
    ...
    this.increment = paramObj.increment;
    ...
    function init()
    {
        ...
        this.slider.bind( "slidechange", function(event, ui)
        {
            ...
            function avanzaPlay()
            {
                that.myLog("Avanzo el slider auto",3);
                that.myLog("El nowDate es ",3);
                that.myLog(that.nowDate,3);

                //console.log("Haciendo play");

                if((that.playing==true) && (that.nowDate<that.endDate))
                {

//                    console.log("Incremento fecha");
                    // Añado un día a la nowDate

                    that.nowDate = that.nowDate.clone().add('days',that.increment);
...
//                    console.log(that.nowDate);
                    ...
                    $( that.parentSelect + " .slider" ).slider('value', $( that.parentSelect + " .slider" ).slider('value') + that.increment);
                }
